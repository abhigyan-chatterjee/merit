...
def generate_core_questions(count: int = 100, seed: int = 202) -> list[GeneratedQuestion]:
    ...
    core_cases = [
        ...
        ("Where should rate limiting live for a public API gateway?", "At the edge/gateway with a token-bucket per key", ["In each browser tab", "Only in batch jobs", "Inside the database trigger"], "Edge enforcement protects all backends uniformly.", "Hard", "design-scaling"),
